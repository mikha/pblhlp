...
case class RangeControlView(label: String, from: Int, to: Int, initial: Int) {
  private var _value: Int = initial
  def currentValue: Int = _value

  private var changeTrigger: () => Unit = RangeControlView.NoOpTrigger
  def setChangeTrigger(t: () => Unit): Unit =
    changeTrigger = t

  val rangeInput = input(
    `type` := "range",
...
object RangeControlView {
  val NoOpTrigger: () => Unit = () => ()
}
